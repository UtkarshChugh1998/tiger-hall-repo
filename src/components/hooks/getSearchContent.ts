...
export const getSearchContent = (searchVal: string, tabVal: string, setData: Function, setLoading: Function, setError: Function) => {
    ...
    switch (tabVal) {
        ...
        case ContentType.CATEGORY: {
            subQuery = categoryFragment()
            filterContentType = FilterContentType.CATEGORY
            break;
        }
        case ContentType.LIFE_GOAL: {
            subQuery = lifeGoalFragment()
            filterContentType = FilterContentType.LIFE_GOAL
            break;
        }
    }
    const query = `{
